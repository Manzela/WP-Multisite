...
class EzImportController
{
    ...
    public function processProduct($product_data)
    {
        try {
            ...
            elseif (!empty($product_data['seo']) && !empty($product_data['seo']['source_url'])) {
                ...
                if ($query->have_posts()) {
                    ...
                    $stored_urls = $wpdb->get_col("
                        SELECT meta_value 
                        FROM {$wpdb->postmeta} 
                        WHERE meta_key = '_network_source_url' 
            ...
            update_post_meta($product_id, '_network_display_rank', $display_rank);
            ...
            if (!empty($product_data['seo']) && is_array($product_data['seo'])) {
                ...
                if (isset($product_data['seo']['meta_title'])) {
                    update_post_meta($product_id, '_network_meta_title', sanitize_text_field($product_data['seo']['meta_title']));
                    error_log('Set meta title: ' . $product_data['seo']['meta_title']);
                }
                
                // Meta description
                if (isset($product_data['seo']['meta_description'])) {
                    update_post_meta($product_id, '_network_meta_description', sanitize_text_field($product_data['seo']['meta_description']));
                    error_log('Set meta description: ' . $product_data['seo']['meta_description']);
                }
                
                // Focus keywords
                if (isset($product_data['seo']['focus_keywords'])) {
                    update_post_meta($product_id, '_network_focus_keywords', sanitize_text_field($product_data['seo']['focus_keywords']));
                    error_log('Set focus keywords: ' . $product_data['seo']['focus_keywords']);
                }
                
                // Canonical URL
                if (isset($product_data['seo']['canonical_url'])) {
                    update_post_meta($product_id, '_network_canonical_url', esc_url_raw($product_data['seo']['canonical_url']));
                    error_log('Set canonical URL: ' . $product_data['seo']['canonical_url']);
                }
                
                // Redirect to
                if (isset($product_data['seo']['redirect_to'])) {
                    update_post_meta($product_id, '_network_redirect_to', esc_url_raw($product_data['seo']['redirect_to']));
                    error_log('Set redirect to: ' . $product_data['seo']['redirect_to']);
                }
                
                // Redirect type
                if (isset($product_data['seo']['redirect_type'])) {
                    update_post_meta($product_id, '_network_redirect_type', sanitize_text_field($product_data['seo']['redirect_type']));
                    error_log('Set redirect type: ' . $product_data['seo']['redirect_type']);
                }
                
                // Image alt
                if (isset($product_data['seo']['image_alt'])) {
                    update_post_meta($product_id, '_network_image_alt', sanitize_text_field($product_data['seo']['image_alt']));
                ...
                if (isset($product_data['seo']['image_alt_tag']) && !isset($product_data['seo']['image_alt'])) {
                    update_post_meta($product_id, '_network_image_alt', sanitize_text_field($product_data['seo']['image_alt_tag']));
                    error_log('Set image alt from image_alt_tag: ' . $product_data['seo']['image_alt_tag']);
                }
                // Source URL
                if (isset($product_data['seo']['source_url'])) {
                    update_post_meta($product_id, '_network_source_url', $product_data['seo']['source_url']);
