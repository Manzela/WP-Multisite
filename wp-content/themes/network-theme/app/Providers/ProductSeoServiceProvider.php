...
class ProductSeoServiceProvider extends ServiceProvider
{
    ...
    public function addCanonicalTag()
    {
        ...
        if (is_404()) {
            ...
            global $post;
            $custom = get_post_meta($post->ID, '_network_canonical_url', true);
...
ROBOTS;
    ...
    {
        add_meta_box('network_product_seo', 'Network SEO', [$this, 'renderProductSeoMetaBox'], 'product', 'normal', 'high');
    }
    ...
    {
        wp_nonce_field('network_product_seo_nonce', 'network_product_seo_nonce');
        echo view('partials.admin.product-seo', [
            'meta_title' => get_post_meta($post->ID, '_network_meta_title', true),
            'meta_description' => get_post_meta($post->ID, '_network_meta_description', true),
            'focus_keywords' => get_post_meta($post->ID, '_network_focus_keywords', true),
            'canonical_url' => get_post_meta($post->ID, '_network_canonical_url', true),
            'redirect_to' => get_post_meta($post->ID, '_network_redirect_to', true),
            'redirect_type' => get_post_meta($post->ID, '_network_redirect_type', true),
            'image_alt' => get_post_meta($post->ID, '_network_image_alt', true),
            'source_url' => $this->processHebrewUrl(get_post_meta($post->ID, '_network_source_url', true)),
            'display_rank' => get_post_meta($post->ID, '_network_display_rank', true) ?: 1
        ]);
    }

    public function saveProductSeoFields($post_id)
    {
        if (!isset($_POST['network_product_seo_nonce']) || !wp_verify_nonce($_POST['network_product_seo_nonce'], 'network_product_seo_nonce'))
            return;
        ...
        $fields = ['_network_meta_title', '_network_meta_description', '_network_focus_keywords', '_network_canonical_url', '_network_redirect_to', '_network_redirect_type', '_network_image_alt', '_network_source_url', '_network_display_rank'];

        foreach ($fields as $field) {
            if (isset($_POST[$field])) {
                $value = $_POST[$field];
                if ($field === '_network_display_rank') {
                    $value = max(1, min(10, absint($value)));
                } elseif ($field !== '_network_source_url') {
    ...
    public function outputOpenGraphTags()
    {
        ...
        $s_title = get_post_meta($post->ID, '_network_meta_title', true) ?: get_the_title();
        $s_desc = get_post_meta($post->ID, '_network_meta_description', true) ?: get_the_excerpt();
        $s_img_alt = get_post_meta($post->ID, '_network_image_alt', true);
