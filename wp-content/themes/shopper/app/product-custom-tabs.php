...
// only for "giron-mall.naaman.shop" sub-domain
// add specific text to the end of the description tab
add_filter('woocommerce_product_tabs', function($tabs) {
    if(strpos(esc_url($_SERVER['HTTP_HOST']), 'giron-mall.naaman.shop') !== false) {
        if (is_product()) {
            $product = wc_get_product(get_the_ID());
            $additional_text = "להשיג עכשיו בחנות שלנו בקניון גירון אשקלון! גלו מבצעים בלעדיים והטבות מיוחדות שקיימות רק בסניף המקומי שלנו בקניון גירון. בואו להנות מחווית קנייה מקומית";
