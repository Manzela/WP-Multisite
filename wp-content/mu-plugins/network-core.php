 ...
 */
if (!function_exists('network_is_subsite_path')) {
    function network_is_subsite_path()
    {
        if (is_admin())
            return false;
        $req = $_SERVER['REQUEST_URI'] ?? '/';

        // Multilingual Support for /tiendas/, /stores/, /example-locale-stores/
        $bases = ['/tienda/', '/tiendas/', '/store/', '/stores/', '/<url-encoded-locale-stores>/'];
 ...
 * Triggered on: admin_init, wp_initialize_site
 */
if (!function_exists('network_enforce_homepage_settings')) {
    function network_enforce_homepage_settings($site_id = null)
    {
        $switched = false;
        ...
        if (!$is_main) {
            ...
            if ($bad_page) {
                ...
                error_log("NETWORK CLEANUP: Deleted 'Sites Directory' from Subsite " . get_current_blog_id());
            }
        }

        $target_slug = $is_main ? 'sites-directory' : 'shop';
        $target_titles = $is_main ? ['Sites Directory'] : ['Shop', 'Tienda', 'Store', '<your-locale-stores>']; // Multilingual Fallbacks
        ...
        if ($page_id > 0) {
            ...
            if (get_option('page_on_front') != $page_id || get_option('show_on_front') !== 'page') {
                ...
                error_log("NETWORK ENFORCED: Homepage set to ID $page_id (" . ($is_main ? "Directory" : "Shop") . ") on Site " . get_current_blog_id());
            }
        } else {
...
if (!function_exists('network_get_store_language_map')) {
    function network_get_store_language_map()
    {
        return [
            'es' => 'tiendas',
            ...
            'en' => 'stores',
            'pt' => 'lojas',
            'he' => '<url-encoded-locale-stores>', // example RTL locale token
        ];
    }
}
...
if (!function_exists('network_derive_lang_from_locale')) {
    function network_derive_lang_from_locale($locale = null)
    {
        $locale = $locale ?: get_locale();
        $lang = substr($locale, 0, 2);

        // Validate against known map
        $map = network_get_store_language_map();
        if (isset($map[$lang])) {
            return $lang;
        }
 ...
 */
if (!function_exists('network_build_virtual_path')) {
    function network_build_virtual_path($mega_slug, $locale = null)
    {
        $lang = network_derive_lang_from_locale($locale);
        $map = network_get_store_language_map();
 ...
 */
if (!function_exists('network_get_clean_mega_slug')) {
    function network_get_clean_mega_slug($dirty_slug, $blog_id = null)
    {
        ...
        // Fix: Extract strict Brand Name if "Store Name" contains location (e.g. "example-tenant-d Example City" -> "example-tenant-d")
...
if (!function_exists('network_enforce_mega_slug')) {
    function network_enforce_mega_slug($site_id = null)
    {
        ...
        // Example: "ExampleSports example-city-example-street" -> "example-city-example-street"
        ...
        $clean_mega_slug = network_get_clean_mega_slug(sanitize_title($site_title), $blog_id);
        ...
        if ($current_slug_tip !== $clean_mega_slug) {
            ...
            error_log("NETWORK HEALED: Site $blog_id moved to $clean_mega_slug (Source: Title '$site_title')");
        }
...
add_action('admin_init', function () {
    ...
    if ($pagenow === 'options-reading.php' || $pagenow === 'index.php') {
        network_enforce_homepage_settings();
        network_enforce_mega_slug();
...
add_action('wp_initialize_site', function ($new_site) {
    ...
    $theme_dir = WP_CONTENT_DIR . '/themes/network-theme';
    if (file_exists($theme_dir . '/vendor/autoload.php')) {
        ...
        if (class_exists('App\Providers\GoogleBusinessServiceProvider')) {
            try {
                ...
                error_log("NETWORK AUTO-SETUP: GMB Sync Result for Site {$new_site->blog_id}: " . json_encode($result));
            } catch (\Throwable $e) {
                error_log("NETWORK AUTO-SETUP ERROR: " . $e->getMessage());
            }
        }
    }

    $site_title = get_option('blogname');
    $brand_slug = network_get_clean_mega_slug(sanitize_title($site_title), $new_site->blog_id);
    ...
    // Uses network_get_store_language_map() as single source of truth.
    $new_path = network_build_virtual_path($brand_slug, $wplang);
    ...
    network_enforce_homepage_settings();
    // network_enforce_mega_slug() is redundant as we just set it, but good safety.
    network_enforce_mega_slug();
...
if (!function_exists('network_set_wc_completed_state')) {
    function network_set_wc_completed_state()
    {
...
add_action('wp_initialize_site', function ($new_site) {
    switch_to_blog($new_site->blog_id);
    network_set_wc_completed_state();
    restore_current_blog();
}, 20);
...
// in network_set_wc_completed_state() are sufficient to prevent task list redirects.
...
add_action('ns_cloner_process_finish', function () {
    ...
    $clean_mega_slug = network_get_clean_mega_slug(sanitize_title($site_title), $target_id);
    update_option('store_mega_slug', $clean_mega_slug);
    ...
    $new_path = network_build_virtual_path($clean_mega_slug, $source_lang);
    $new_url = rtrim(network_site_url($new_path), '/');
    update_option('siteurl', $new_url);
    update_option('home', $new_url);
    ...
    error_log("NETWORK NS-CLONER: Post-clone restoration complete for Site {$target_id} (Slug: {$clean_mega_slug})");
}, 20);
 ...
 */
class Network_Permalink_Manager
{
...
Network_Permalink_Manager::init();
